...
class ScreenRecorder: ObservableObject {
    ...
    @Published var delaySelectorIndex = 0{
        didSet{
            delaySelectorIndex %= NSScreen.screens.count
            selectedDisplay = availableDisplays[delaySelectorIndex]
        }
    }
    
    @Published var delayFullScreen = false{
        didSet{
            
            self.configMinMax = delayFullScreen ? 0.0 : 280.0
            self.configPadding = delayFullScreen ? 0.0 : 8
            
            DispatchQueue.main.async {
                let delegate = NSApp.delegate as! AleDelegate
                delegate.onFullScreen(self.delayFullScreen)
            }

        }
    }
    
    @Published var delaySeconds = 0.0{
    ...
    @Published var configMinMax : CGFloat = 280.0
    @Published var configPadding : CGFloat  = 8.0
    ...
    var canRecord: Bool {
        ...
        //get foo = captureEngine.pixelBufferDelay.delayInSeconds
    }
...
//        streamConfig.minimumFrameInterval = CMTime(value: 1, timescale: 30) // trying 24 fps
